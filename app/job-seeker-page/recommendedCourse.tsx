...
export default function LearningInformation() {
  ...
  const jobSkillArray = useMemo(() => {
    if (!jobSkill) return [] as string[];

    // If jobSkill is already an array
    if (Array.isArray(jobSkill)) {
      return jobSkill
        .flatMap((s) => s.split(","))
    ...
    if (typeof jobSkill === "string") {
      try {
        const parsed = JSON.parse(jobSkill);
          ...
          {skillsLoading ? (
            ...
            <View style={styles.skillChipContainer}>
              {skills.map((skill, index) => {
                ...
                const isMatched = jobSkillArray.includes(normalizedSkill);
                  ...
                  >
                    <Text
                      style={[
                        ...
                        isMatched && styles.skillChipMatchedText,
                      ]}
                    >
                      {skill}
                    </Text>
