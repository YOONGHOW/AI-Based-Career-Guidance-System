...
export default function LearningInformation() {
  ...
  const [registedDocId, setRegistedDocId] = useState<string | null>(null);
  ...
  useEffect(() => {
    const fetchCourse = async () => {
      try {
        ...
        if (!regSnap.empty) {
          setRegistedDocId(regSnap.docs[0].id);
        } else {
  ...
  const deleteRegistedCourse = async () => {
    if (!registedDocId) {
    ...
    try {
      await deleteDoc(doc(db, "courseRegisted", registedDocId));
      await updateDoc(doc(db, "users", uid), {
        courseRegister_list: arrayRemove(courseId),
      });
      Alert.alert("Removed", "Course unregistered successfully");
  ...
  const handleCompleteCourse = async () => {
    if (!user || !courseId || !course) {
      Alert.alert("Error", "User or course not found");
      return;
    }
    ...
    try {
      ...
      if (historySnap.empty) {
        await addDoc(collection(db, "courseHistory"), {
          userId: uid,
          course_id: courseId,
          courseTitle: course.course_title,
          provider_names: course.provider_names || [],
          courseStatus: "Completed",
          completedAt: new Date(),
        });
      }

      //Merge skills into skillProfile
      if (skills.length > 0) {
        const spQuery = query(
        ...
        if (!spSnap.empty) {
          ...
          const currentSkills: string[] = Array.isArray(spData.skills)
            ? spData.skills
            : [];

          const merged = Array.from(new Set([...currentSkills, ...skills]));

          await updateDoc(doc(db, "skillProfile", spDoc.id), {
            skills: merged,
          });
        } else {
          await addDoc(collection(db, "skillProfile"), {
            userId: uid,
            skills: skills,
            createdAt: new Date(),
          });
      ...
      // 3️⃣ NEW: delete from courseRegisted + user.courseRegister_list
      if (registedDocId) {
        await deleteDoc(doc(db, "courseRegisted", registedDocId));
      ...
      Alert.alert(
        "Congratulations!",
        "Course marked as completed and skills added to your profile!"
      );
      router.push("/(job-seekerTabs)/learn");
    } catch (err) {
      console.error("Complete failed:", err);
      Alert.alert("Error", "Unable to complete this course.");
    } finally {
      setCompleting(false);
    }
  };
